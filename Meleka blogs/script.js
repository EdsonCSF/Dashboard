...
//  CORREÇÃO PRINCIPAL: Aplica filtro com suporte a FILMES + SÉRIES futuras
async function applyFilter(filterType) {
    let endpoint = '';
    let fetchTV = false; // Flag para buscar séries separadamente
    ...
    switch(filterType) {
        ...
        case 'tv':
            ...
            break;
        case 'upcoming':
            //  BUSCA FILMES FUTUROS + SÉRIES EM EXIBIÇÃO
            const [movieData, tvData] = await Promise.all([
                fetchFromAPI('/movie/upcoming?region=US', 1),
                fetchFromAPI('/tv/on_the_air?region=US', 1)
            ]);
            ...
            renderCards(combinedResults, 'filter-results', false);
            return; // Sai aqui porque já renderizamos
        case 'trending':
